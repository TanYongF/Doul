...
### 使用指南
1. 安装Mysql、Redis、Etcd等系统组件（如需上传视频还需配置阿里云OSS）
2. 在mysql中执行`ddl.sql`建立表结构
3. 在各个微服务模块中配置相应的`rpc`和`api`配置文件
4. 启动各个微服务 按照用户中心->安全中心->视频中心->评论中心 顺序一次启动，首先启动`rpc`服务，再启动`api`服务
